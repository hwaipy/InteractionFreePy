...
A *Worker* can have a unique name that can be addressed by other *Workers* (also called *Service*), or anonymous (also called *Client*).

Just like a normal server-client mode, the *Service* provides certain service, and the *Client* consumes the service.
The only difference is that the *Client* does not need to know the address of the *Service*, or access the *Service* directly.
They just need to know the *Broker* address and the unique name of the *Service*, and the *Broker* will take care of the rest.
...
Here, the yellow circle presents the named *Worker* (*Service*), and the green circles present the anonymous *Workers* (*Clients*).
...
For example, if a *Service* named `DragonCipher` provides a function `encrypt_to_dragon_speech`, the call would be like this:
...
When any *Worker* makes the above invocation, it is actually the function in the *Service* `DragonCipher` that is executed.
...
The functions available in the *Broker* are listed in the API documentation {py:class}`Manager <interactionfreepy.broker.Manager>`.
